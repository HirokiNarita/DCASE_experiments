...
def get_id(wav_names):
    ...
    for wav_name in wav_names:
        ...
        id_types.append(int(str_id[4]))
        
    
    return np.array(id_types)
...
def calc_DCASE2020_score(all_scores_df, labels, preds, section_types, wav_names):
    describe_df = get_pred_discribe(labels, preds, section_types, wav_names)
    scores_df = get_score_per_Section(describe_df, max_fpr=0.1)
    scores_df = scores_df.rename(index=lambda num: 'id_' + f'{num}')
    all_scores_df = all_scores_df.append(scores_df)
    # 平均
    mean_df = pd.DataFrame(all_scores_df.mean(axis=0)).T
    mean_df.index = ['mean']
